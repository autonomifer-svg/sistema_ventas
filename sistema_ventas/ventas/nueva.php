...
if (isset($_POST['agregar'])) {
    ...
    if ($producto_seleccionado && $cantidad > 0 && $cantidad <= $producto_seleccionado['Stock']) {
        ...
        unset($item); // Romper la referencia para evitar bugs en bucles posteriores.
...
if (isset($_POST['finalizar_venta'])) {
    ...
    if (empty($_SESSION['carrito']) || !$_SESSION['cliente_seleccionado']) {
        ...
        // Habilitar reporte de errores para detectar fallos
        ...
        try {
            ...
            // OBTENER IdTipoCliente DEL CLIENTE
            // Se usa COALESCE para asignar un tipo de cliente por defecto (1) si es nulo,
            // para evitar el error "Column 'IdTipoCliente' cannot be null".
            $sql_cliente_tipo = "SELECT COALESCE(IdTipoCliente, 1) AS IdTipoCliente FROM clientes WHERE NroCliente = ?";
            $stmt_tipo = $conexion->prepare($sql_cliente_tipo);
            $stmt_tipo->bind_param("i", $_SESSION['cliente_seleccionado']);
            $stmt_tipo->execute();
            $res_tipo = $stmt_tipo->get_result();
            $cliente_row = $res_tipo->fetch_assoc();
            // Si el cliente existe, usa su tipo. Si no (caso raro), usa 1 como fallback.
            $tipo_cliente = $cliente_row ? $cliente_row['IdTipoCliente'] : 1;
            $stmt_tipo->close();
            ...
            $sql_venta = "INSERT INTO salida (IdSalida, NroSucursal, NroCliente, Fecha, FechaPedido, Hora, Total, Iva, Estado, IdFormaPago, IdUsuario, IdTipoCliente) 
                          VALUES (?, ?, ?, CURDATE(), CURDATE(), CURTIME(), ?, ?, 1, ?, ?, ?)";
            ...
            // c. PREPARAR INSERT PARA DETALLES
            // Obtenemos el último ID de detalle para incrementarlo manualmente
            $sql_max_detalle = "SELECT COALESCE(MAX(IdDetalleSalida), 0) as ultimo FROM detallesalida";
            $id_detalle_actual = $conexion->query($sql_max_detalle)->fetch_assoc()['ultimo'];

            $sql_detalle = "INSERT INTO detallesalida (IdDetalleSalida, IdSalida, CodigoNum, Descripcion, Cantidad, Preciounidad, Totalin) 
                            VALUES (?, ?, ?, ?, ?, ?, ?)";
            ...
            foreach ($_SESSION['carrito'] as $item) {
                $id_detalle_actual++; // Incrementamos ID para el nuevo detalle
...
<div class="container-fluid">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h2>Nueva Venta</h2>
        <div>
            <a href="historial.php" class="btn btn-info btn-modern me-2"><i class="bi bi-clock-history"></i> Ver
                Historial</a>
            <a href="nueva.php" class="btn btn-outline-secondary btn-modern"><i class="bi bi-arrow-clockwise"></i>
                Reiniciar</a>
        </div>
    </div>
    ...
    <?php if (isset($error)): ?>
        <div class="alert alert-danger alert-dismissible fade show"><i class="bi bi-exclamation-triangle-fill"></i>
            <?= $error ?><button type="button" class="btn-close" data-bs-dismiss="alert"></button></div>
    <?php endif; ?>

    <?php if (isset($_SESSION['mensaje_exito'])): ?>
        <div class="alert alert-success alert-dismissible fade show"><i class="bi bi-check-circle-fill"></i>
            <?= $_SESSION['mensaje_exito'] ?><button type="button" class="btn-close" data-bs-dismiss="alert"></button></div>
                ...
                <div class="card modern-card">
                    ...
                    <div class="card-body p-4">
                        <div class="position-relative mb-4">
                            <input type="text" id="searchCliente" class="form-control search-input"
                                    ...
                                    <div class="col-md-6">
                                        <div class="card cliente-item h-100"
                                            ...
                                            <div class="card-body d-flex align-items-center">
                                                <div
                                                    style="width: 50px; height: 50px; border-radius: 50%; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); display: flex; align-items: center; justify-content: center; color: white; font-weight: bold; font-size: 1.3rem; margin-right: 1rem;">
                                                    <?= strtoupper(substr($cliente['Nombre'], 0, 1)) ?>
                                                </div>
                                                <div class="flex-grow-1">
                                                    <h6 class="cliente-nombre mb-1"><?= htmlspecialchars($cliente['Nombre']) ?>
                                                    </h6>
                                                ...
                                                <form method="POST">
                                                    ...
                                                    <button type="submit" name="seleccionar_cliente"
                                                        class="btn btn-gradient btn-modern"><i class="bi bi-check-circle"></i>
                                                        Seleccionar</button>
                                                </form>
                                            </div>
                                        </div>
        ...
        <div class="row">
            <div class="col-lg-7">
                <div class="client-info">
                    <div class="d-flex justify-content-between align-items-center">
                        ...
                        <form method="POST">
                            <button type="submit" name="cambiar_cliente" class="btn btn-light btn-modern"><i
                                    class="bi bi-arrow-left-circle"></i> Cambiar</button>
                        </form>
                    </div>
                </div>
                                    ...
                                    <select name="sucursal_id" class="form-select" required>
                                        <?php foreach ($sucursales as $sucursal): ?>
                                            <option value="<?= $sucursal['NroSucursal'] ?>"
                                                <?= $_SESSION['sucursal_seleccionada'] == $sucursal['NroSucursal'] ? 'selected' : '' ?>><?= htmlspecialchars($sucursal['Nombre']) ?></option>
                                        <?php endforeach; ?>
                                    </select>
                                </div>
                                <div class="col-md-6">
                                    ...
                                    <select name="forma_pago_id" class="form-select" required>
                                        <?php foreach ($formas_pago as $forma): ?>
                                            <option value="<?= $forma['IdFormaPago'] ?>"
                                                <?= $_SESSION['forma_pago_seleccionada'] == $forma['IdFormaPago'] ? 'selected' : '' ?>><?= htmlspecialchars($forma['FormaPago']) ?></option>
                                        <?php endforeach; ?>
                                    </select>
                                </div>
                                <div class="col-12">
                                    <button type="submit" name="actualizar_config"
                                        class="btn btn-gradient btn-modern w-100"><i class="bi bi-check-circle"></i>
                                        Actualizar</button>
                                </div>
                            </div>
                        </form>
                ...
                <div class="card modern-card">
                    <div class="card-header gradient-header">
                        <h5 class="mb-0"><i class="bi bi-plus-circle"></i> Agregar Productos</h5>
                    </div>
                    <div class="card-body">
                        <?php if (empty($productos)): ?>
                            <div class="alert alert-warning"><i class="bi bi-exclamation-triangle"></i> No hay productos con
                                stock</div>
                        <?php else: ?>
                            <form method="POST">
                                <div class="row g-3">
                                        ...
                                        <select name="producto_id" class="form-select" required>
                                            ...
                                            <?php foreach ($productos as $producto): ?>
                                                <option value="<?= $producto['CodigoNum'] ?>"
                                                    ...
                                                    $<?= number_format($producto['PrecioVenta'], 0) ?> (Stock:
                                                    <?= $producto['Stock'] ?>)
                                                </option>
                                            <?php endforeach; ?>
                                        </select>
                                    ...
                                    <div class="col-md-3">
                                        ...
                                        <input type="number" name="cantidad" class="form-control quantity-input" min="1"
                                            step="0.01" value="1" required>
                                    </div>
                                    <div class="col-md-2 d-flex align-items-end">
                                        <button type="submit" name="agregar" class="btn btn-gradient btn-modern w-100"><i
            ...
            <div class="col-lg-5">
                <div class="card modern-card sticky-top" style="top: 20px;">
                    <div class="card-header gradient-header">
                        <h5 class="mb-0"><i class="bi bi-cart3"></i> Carrito</h5>
                    </div>
                    <div class="card-body">
                        ...
                        <?php else: ?>
                                        ...
                                        <div class="d-flex justify-content-between align-items-center">
                                            <form method="POST" class="d-flex align-items-center">
                                                ...
                                                <input type="number" name="nueva_cantidad"
                                                    class="form-control form-control-sm quantity-input" style="width: 70px;" min="1"
                                                    max="<?= $item['stock'] ?>" step="0.01" value="<?= $item['cantidad'] ?>">
                                                <button type="submit" name="actualizar_cantidad"
                                                    class="btn btn-sm btn-outline-primary ms-2"><i class="bi bi-check"></i></button>
                                            </form>
                            ...
                            <form method="POST" class="mt-3">
                                <button type="submit" name="finalizar_venta"
                                    class="btn btn-success-gradient btn-modern w-100"><i class="bi bi-check-circle"></i>
